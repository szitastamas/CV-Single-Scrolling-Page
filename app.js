...
window.onscroll = function() {
    ...
    isSectionInViewport(showcase) ? homeNav.classList.add('current') : homeNav.classList.remove('current');
    isSectionInViewport(aboutMe) ? aboutNav.classList.add('current') : aboutNav.classList.remove('current');
    isSectionInViewport(education) ? eduNav.classList.add('current') : eduNav.classList.remove('current');
    isSectionInViewport(experience) ? expNav.classList.add('current') : expNav.classList.remove('current');
    isSectionInViewport(contactMe) ? contactNav.classList.add('current') : contactNav.classList.remove('current');
...
const isSectionInViewport = el => {
    ...
    return rect.bottom > 550 &&
        rect.right > 0 &&
        rect.left <= (window.innerWidth || document.documentElement.clientWidth) &&
        rect.top <= ((window.innerHeight - 350) || (document.documentElement.clientHeight - 350));
}
